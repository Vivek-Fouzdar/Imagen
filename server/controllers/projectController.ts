...
export const createProject = async (req:Request, res:Response)=>{
    ...
    if(images.length < 2 || !productName){
        return res.status(400).json({message: 'At least 2 images and product name are required'})
    }
...
if(!user || user.credits < 5){
    ...
    try{
        ...
        const prompt = {
            text:`Combine the person and product into a realistic photo.Make the person naturally hold or use the product. Match lighting, Shadows, scale and perspective. Make the person stand in professional studio lighting.Output ecommerce-quality photo reaslistic imagery.`
        }
     ...
     const {userId} = req.auth()
        ...
        try{
            ...
            await prisma.project.update({
                where: {id: projectId},
                data: {
                    generatedVideo: uploadResult.secure_url,isGenerating: false
                }
            })
